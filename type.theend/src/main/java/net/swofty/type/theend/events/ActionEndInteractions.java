...
public class ActionEndInteractions implements HypixelEventClass {
    ...
    private static final Pos DRAGON_ALTAR = new Pos(-600, 22, -275);
    ...
    public void run(PlayerBlockInteractEvent event) {
        ...
        if (event.getBlockPosition().distanceSquared(DRAGON_ALTAR) > 16) return;
        ...
        event.setCancelled(true);
        if (!EndDragonManager.placeEye(player)) return;
